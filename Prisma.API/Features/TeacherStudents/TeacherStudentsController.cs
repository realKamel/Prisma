...
public class TeacherStudentsController(IMediator mediator) : ApiController
{
    ...
    [HttpGet("{id:guid}")]
    public async Task<Result<StudentListItemDto>> GetById(
        Guid id,
    ...
    public async Task<Result<List<StudentLessonDto>>> GetLessons(
        Guid id,
        CancellationToken cancellationToken
    )
    {
    ...
    public async Task<Result<List<StudentActivityDto>>> GetActivities(
        Guid id,
        CancellationToken cancellationToken
    )
    {
    ...
    public async Task<Result<StudentStatsDto>> GetStats(
        Guid id,
        CancellationToken cancellationToken
    )
    {
    ...
    public async Task<Result<List<LessonForGrantDto>>> GetLessonsForGrant(
        CancellationToken cancellationToken
    )
    {
    ...
    [HttpGet("lessons")]
    public async Task<Result<List<LessonForGrantDto>>> GetAllLessons(
        CancellationToken cancellationToken
    )
    {
        var result = await mediator.Send(new GetTeacherLessonsForGrantQuery(), cancellationToken);
    ...
    public async Task<Result<List<AcademicYearOptionDto>>> GetAcademicYears(
        CancellationToken cancellationToken
    )
    {
    ...
    [HttpPost("grant")]
    [EnableRateLimiting(RateLimitPolicies.UserWrite)]
    public async Task<Result> GrantLesson(
        [FromBody] GrantLessonRequest request,
        CancellationToken cancellationToken
    )
    {
    ...
    [EnableRateLimiting(RateLimitPolicies.UserWrite)]
    public async Task<Result> RevokeLesson(
        Guid studentId,
        int lessonId,
        CancellationToken cancellationToken
    )
    {
        var result = await mediator.Send(
            new RevokeLessonCommand(studentId, lessonId),
            cancellationToken
        );
        return result;
    }
    ...
    [EnableRateLimiting(RateLimitPolicies.UserWrite)]
    public async Task<Result> SendReport(
