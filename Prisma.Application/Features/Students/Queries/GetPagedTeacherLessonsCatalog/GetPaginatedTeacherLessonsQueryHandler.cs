...
public class GetPaginatedTeacherLessonsQueryHandler(
...
) : IRequestHandler<GetPaginatedTeacherLessonsQuery, Result<PaginatedList<LessonCatalogDto>>>
{
    ...
    )
    {
        ...
        var lessons = await lessonRepo.ListAsync(
            new PagedTeacherLessonsCatalogSpecification(
                request.Id,
                request.Keyword,
        ...
        var lessonCount = await lessonRepo.CountAsync(
            new PagedTeacherLessonsCatalogSpecification(
                request.Id,
                request.Keyword,
                student.AcademicYearId.Value
    ...
    private static LessonCatalogStatus DetermineStatus(
        Lesson lesson,
        Guid studentId,
        ICollection<Lesson> allLessons
    )
