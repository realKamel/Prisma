...
{
    public async Task<Result<AccentColorDto>> Handle(
        GetAccentColorQuery request,
        CancellationToken ct
    )
    {
        var userRepository = unitOfWork.GetOrCreateRepository<User, Guid>();
        var teacher = await userRepository.FirstOrDefaultAsync(
            new UserByEmailSpecification(request.TeacherEmail),
            ct
        );
        ...
        var preferences = await repository.FirstOrDefaultAsync(
            new TeacherPreferencesByTeacherIdSpecification(teacher.Id),
            ct
        );
